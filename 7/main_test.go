...
func TestExamples(t *testing.T) {
	...
	maxThrust := findMaxThrust(program, false)
...
func TestFeedbackLoop(t *testing.T) {
	...
	maxThurst := findMaxThrust(program, true)

	assert.Equal(139629729, maxThurst)
}

func TestAltFeedbackLoop(t *testing.T) {
	assert := assert.New(t)
